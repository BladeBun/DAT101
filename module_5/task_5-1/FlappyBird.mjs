...
export const GameProps = {
    ...
    score: 0,
    bestScore: 0,
    sounds: {countDown: null, food: null, gameOver: null, dead: null, running: null, flapping: null},
};
...
function loadGame() {
    ...
    GameProps.sounds.flapping = new libSound.TSoundFile("./Media/Flapping.mp3");
...
function setDayNight() {
    if (rbDayNight[0].checked) {
        ...
        SpriteInfoList.obstacle.x = 0;
    } else {
        GameProps.dayTime = false;
        SpriteInfoList.background.x = 246 + 573;
        console.log("Night time");
        SpriteInfoList.obstacle.x = -52 * 2;
    }
} // end of setDayNight
...
function onKeyDown(aEvent) {
    switch (aEvent.code) {
        case "Space":
            if (!GameProps.hero.isDead) {
                GameProps.hero.flap();
                playSound(GameProps.sounds.flapping);
            }
            break;
    }
}
